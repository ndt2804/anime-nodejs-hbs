...
class AnimeController {
        ...
        Anime.find({ _id: { $ne: anime._id }, genres: { $in: anime.genres } })
          ...
          .then((recommendations) => {
            ...
            const chungCount = recommendations.map((video) => {
              const commonGenres = video.genres.filter((genre) =>
            ...
            const recommendedVideos = chungCount
              .slice(0, 5)
              .map((item) => item.video);
  ...
  createEpisodes(req, res, next) {
    Anime.find({})
      .then((animes) => {
        res.render("animes/createEpisodes", {
          animes: multiMongooseToObj(animes),
        });
      })
      .catch(next);
